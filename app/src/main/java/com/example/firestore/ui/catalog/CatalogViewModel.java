...
public class CatalogViewModel extends AndroidViewModel {
    ...
    public void resetPassword(String email) {
        ...
        repo.resetPassword(email.trim(), new UserRepository.AuthCallback() {
            @Override public void onSuccess(FirebaseUser user) {
                authState.postValue(AuthState.error("Correo de recuperaciÃ³n enviado."));
            }
            @Override public void onError(String message) {
                authState.postValue(AuthState.error(message));
            }
    ...
    private String validate(String email, String password, String confirmPassword, boolean isRegister) {
        ...
        if (isRegister) {
            ...
            if (!email.matches(emailPattern)) return "Formato de correo electrÃ³nico no vÃ¡lido.";
            if (password == null || password.isEmpty()) return "La contraseÃ±a es obligatoria.";
            if (password.length() < 8) return "La contraseÃ±a debe tener al menos 8 caracteres.";
            if (!password.matches(".*[a-zA-Z].*")) return "La contraseÃ±a debe contener al menos una letra.";
            if (!password.matches(".*[0-9].*")) return "La contraseÃ±a debe contener al menos un nÃºmero.";
