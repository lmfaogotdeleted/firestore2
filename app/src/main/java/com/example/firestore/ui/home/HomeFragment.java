...
observarPendientesYSaludar();
    ...
    private void mostrarDialogo1() {
        ...
        new AlertDialog.Builder(requireContext())
                ...
                .setMessage("â€œHola! Para personalizar tu experiencia, puedes configurar tu nombre en los ajustes.\n" +
                        "Â¿Quieres hacerlo ahora?â€")
                .setCancelable(false)
    ...
    private void mostrarDialogoBienvenidaPersonalizado(String nombre, MediaItem item) {
        new AlertDialog.Builder(requireContext())
                .setTitle("Sugerencia")
                .setMessage("Hola, " + nombre + ". Â¿Has visto ya tu pelÃ­cula o serie pendiente " + item.getTitulo() + "?")
                .setCancelable(true)
                .setPositiveButton("AÃ±adir TrackingEntity", (dialog, which) -> {
                    viewModel.selectDesdeFavoritos(item);
                    navController.navigate(R.id.action_InicioFragment_to_pendientesFragment);
                })
                .setNegativeButton("AÃºn no", (dialog, which) -> dialog.dismiss())
                .show();
    }
