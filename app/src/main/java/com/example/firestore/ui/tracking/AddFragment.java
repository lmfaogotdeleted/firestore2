...
public class AddFragment extends Fragment {
    ...
    public void onViewCreated(@NonNull View view, @Nullable Bundle savedInstanceState) {
        ...
        binding.btnSearchTMDB.setOnClickListener(v -> {
            ...
            if (query.isEmpty()) {
                Toast.makeText(getContext(), "Introduce un tÃ­tulo para buscar", Toast.LENGTH_SHORT).show();
                return;
            }
    ...
    private void anadirAnimal() {
        if (binding.spinnerResults.getSelectedItem() == null) {
            Toast.makeText(getContext(), "Primero busca y selecciona un tÃ­tulo", Toast.LENGTH_SHORT).show();
            return;
        }
        String nombre = binding.spinnerResults.getSelectedItem().toString();
        String fecha = binding.etDate.getText().toString();
        if (fecha.isEmpty()) {
            Toast.makeText(getContext(), "Selecciona una fecha de visualizaciÃ³n", Toast.LENGTH_SHORT).show();
            return;
        }
        String tipoSeleccionado = "";
        int checkedId = binding.toggleGroupType.getCheckedButtonId();
        if (checkedId == R.id.btnTypeMovie) {
            tipoSeleccionado = "PelÃ­cula";
        } else {
            tipoSeleccionado = "Serie";
        }
        float puntuacion = binding.ratingScore.getRating();
        String urlImagenApi = "";
        if (tipoSeleccionado.equals("PelÃ­cula") && CatalogViewModel.Peliculas.getValue() != null) {
